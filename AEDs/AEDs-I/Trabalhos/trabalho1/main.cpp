...
/*
    Autores:
    - Diego Vianna Leite Montemor – RA: 2025.1.08.023
    - Guilherme de Oliveira Aredes – RA: 2025.1.08.026

    Descrição:
    Este programa é um sistema de gerenciamento de imóveis que permite a inclusão, busca, edição e exclusão de imóveis em um banco de dados.
*/
...
int menu(int quant) { // Menu principal
    int opcao;
...
void menuInclusao(int quant, Imovel Vetor[]){
    ...
    cin >> Op;
    while(Op < 1 and 5 < Op){ // Verifica se a opção é válida
        cout << "Opção invalida, digite uma opção valida:";
        cin >> Op;
    }
    if(Op == 1){ // relaciona a opção com o tipo do imóvel 
        strcpy(Tipo, "casa"); 
    }else if(Op == 2){
    ...
    if(OpF == 1){ // relaciona a opção com a finalidade do imóvel
    ...
    if(OpF == 1){ // Se for venda passa apenas o valor do imóvel
...
void excluirImovel(Imovel imoveis[], int &total){ // menu para excluir um imóvel
    ...
    for (int i = 0; i<total; i++){
        ...
        switch (opcao){  // Verifica o critério de exclusão
        ...
        if (deveExcluir){ // Se o critério de exclusão for atendido
            encontrado = true;
...
void relatorio(int totalImoveis,int cVenda,int cLocacao,int totalCasas,int casas_com_suite,int salacomercial_totais,int salacomercial_ceramica) {
    ...
    if (totalImoveis == 0) { // caso de nenhum imóvel cadastrado ele retorna para o menu
...
void listarImoveis(Imovel Imoveis[], int quantidade) { // Função para listar todos os imóveis cadastrados
    for (int i = 0; i < quantidade; i++) { // Percorre o vetor de imóveis e exibe as informações de cada um
...
int main(){
    ...
    ifstream informacoes("../data/BD_Imoveis2.txt");  // le o arquivo de imóveis
    if (!informacoes) {
    ...
    Imovel Imoveis[200];
    int QuantAtual = 0;
    while (  // caso ele consiga ler todos os dados de um imóvel, ele adiciona no vetor
    ...
    QuantAtual++; // Incrementa o contador de imóveis lidos
    }

    int QuantLeitura;
    if(QuantAtual > 100)  QuantLeitura = 100; // limita a leitura a 100 imóveis
    ...
    while (opcao != 0) {
        switch(opcao) {
            case 1: // opção de inclusão de novo imóvel
                ...
                break;
            case 2:  // opção de busca de imóvel
                cout << "Acessando menu de busca..." << endl;
                menuBusca(QuantLeitura, Imoveis); // chama a função de busca
                break;
             case 3:{  // opção de relatório de imóveis
                informacoes.clear(); // limpa o estado do stream
                informacoes.seekg(0); // Volta a leitura do arquivo para o início
    ...
    ofstream informacoesSaida("../data/BD_Imoveis2.txt"); // Abre o arquivo para escrita
    if (!informacoesSaida) {
